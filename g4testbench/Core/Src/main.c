...
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#define MAX_BLOCKS 64  // Adjust this based on your card's capacity
...
uint8_t Readdata;
// Global variables for debugging and data storage
volatile uint8_t g_currentBlock = 0;
volatile uint8_t g_readStatus = 0;
volatile uint8_t g_authStatus = 0;
...
// Global array to store all read data
volatile uint8_t g_allBlockData[MAX_BLOCKS][16] = {0};
volatile uint8_t g_blockReadStatus[MAX_BLOCKS] = {0};  // 0: Not read, 1: Read successfully
